...
public record GetMemberDetailQuery(int IdMember) : IRequest<Result<MemberDetailDto>>, ITenantRequest
{
    ...
    ILogger<GetMemberDetailHandler> logger) : IRequestHandler<GetMemberDetailQuery, Result<MemberDetailDto>>
{
    public async Task<Result<MemberDetailDto>> Handle(GetMemberDetailQuery request, CancellationToken cancellationToken)
    {
            ...
            .Select(m => new
            {
                ...
                Subscriptions = m.SubscriptionMemberships
                    ...
                    { 
                        ...
                        IsActive = s.IsSubscriptionActive(utcNow)
                    })
                    .ToList()
            })
        ...
        var chainStartDateUtc = activeSubscriptions.Count != 0
            ? activeSubscriptions.Min(s => s.StartDate) 
            : lastSubscription?.StartDate;

        var absoluteEndDateUtc = activeSubscriptions.Count != 0
            ? activeSubscriptions.Max(s => s.EndDate) 
            : lastSubscription?.EndDate;
        
        var displaySub = 
            activeSubscriptions.FirstOrDefault(s => s.StartDate <= utcNow) // La que corre hoy
        ...
        var localJoinedDate = await timeService.ConvertUtcToLocalAsync(member.CreatedAt, request.IdTenant, cancellationToken);
        
        var localChainStart =  chainStartDateUtc.HasValue
            ? await timeService.ConvertUtcToLocalAsync(chainStartDateUtc.Value, request.IdTenant, cancellationToken)
            : (DateTime?)null;

        var localAbsoluteEnd = absoluteEndDateUtc.HasValue
            ? await timeService.ConvertUtcToLocalAsync(absoluteEndDateUtc.Value, request.IdTenant, cancellationToken)
            : (DateTime?)null;

        var metrics = SubscriptionMetrics.Calculate(
            currentDate: localToday,
            joinedDate: localJoinedDate,
            chainStartDate: localChainStart,
            absoluteEndDate: localAbsoluteEnd,
